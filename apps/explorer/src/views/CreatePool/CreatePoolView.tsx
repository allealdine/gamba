...
import { ParsedTokenAccount, useNativeBalance, useTokenList } from '@/hooks';
      ...
      <Flex justify="between">
        <Flex gap="4" align="center">
          ...
          <Flex direction="column">
            <Text>{meta.name}</Text>
            <Text>
              {meta.symbol ?? truncateString(props.token.mint.toString())}
            </Text>
          </Flex>
        </Flex>
...
function PublicPoolWarning({ token }: { token: ParsedTokenAccount }) {
  ...
  return (
    <>
      <Text>
        You are about to create a public liqudity pool for{' '}
        <Avatar
          src={selectedTokenMeta.image}
          fallback="?"
          size="1"
          radius="full"
        />
        <b>
          {selectedTokenMeta.name} ({selectedTokenMeta.symbol})
        </b>
        .
      </Text>
      <Text>
        Since it's public, anyone will be able make deposits to it, and any
        frontend will be able to make use of its liquidity for plays.
      </Text>
      <Text>
        The cost of creating a pool is{' '}
        <b>
          <TokenValue2
            mint={NATIVE_MINT}
            amount={gambaState?.poolCreationFee ?? 0}
          />
        </b>{' '}
        + rent.
      </Text>
...
function PrivatePoolWarning({ token }: { token: ParsedTokenAccount }) {
  ...
  return (
    <>
      <Text>
        You are about to create a private liqudity pool for{' '}
        <Avatar
          src={selectedTokenMeta.image}
          fallback="?"
          size="1"
      ...
      <Text>Please read up on private pools before creating.</Text>
      <Text>
        The cost of creating a pool is{' '}
        <b>
          <TokenValue2
...
function Inner() {
  ...
  const selectedPoolId =
    selectedToken && getPoolAddress(selectedToken.mint, authority);
  ...
  const sortedTokens = React.useMemo(() => {
    return tokens.sort((a, b) => {
      const nativeMintDiff =
        Number(isNativeMint(b.mint)) - Number(isNativeMint(a.mint));
  ...
  const filteredTokens = React.useMemo(
    () =>
      sortedTokens.filter(x => {
        ...
        if (
          x.mint.toBase58().toLocaleLowerCase().includes(search.toLowerCase())
        )
          return true;
        if (meta.symbol?.toLowerCase().includes(search.toLowerCase()))
          return true;
        if (meta.name?.toLowerCase().includes(search.toLowerCase()))
          return true;
        return false;
      }),
        ...
        <Flex direction="column" gap="4">
          ...
          {gambaState && !gambaState.poolCreationAllowed && (
            <Callout.Root color="orange">
              ...
              <Callout.Text>
                Pool Creation is restricted at the moment. Come back later or
                select a pool that has already been created.
              </Callout.Text>
            </Callout.Root>
          )}
          ...
          {!isLoading && !!selectedPool && (
            <Link
              onClick={() =>
                navigate('/pool/' + selectedPool.publicKey.toBase58())
              }
            >
