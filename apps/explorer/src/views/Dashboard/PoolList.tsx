...
const populatePool = async (
  ...
  const { value: lp } = await connection.getTokenSupply(
    getPoolLpAddress(publicKey)
  );
  const underlyingAccount = decodeAta(
    await connection.getAccountInfo(
      getPoolUnderlyingTokenAccountAddress(publicKey)
    )
  );
  const bonusUnderlyingTokenAccount = decodeAta(
    await connection.getAccountInfo(
      getPoolBonusUnderlyingTokenAccountAddress(publicKey)
    )
  );
  const underlyingBalance = underlyingAccount?.amount ?? BigInt(0);
  const bonusBalance = bonusUnderlyingTokenAccount?.amount ?? BigInt(0);
  const jackpotUnderlyingTokenAccount = decodeAta(
    await connection.getAccountInfo(
      getPoolJackpotTokenAccountAddress(publicKey)
    )
  );
...
export async function fetchPool(connection: Connection, publicKey: PublicKey) {
  ...
  // Check if pool is initialized (has non-zero values)
  if (
    pool.liquidityCheckpoint.toNumber() === 0 &&
    pool.plays.toNumber() === 0 &&
    pool.minWager.toNumber() === 0
  ) {
    return null; // Return null for uninitialized pools
  }
...
export function usePopulatedPool(account: ProgramAccount<PoolState>) {
  ...
  return useSWR('populated-pool-' + account.publicKey.toBase58(), () =>
    populatePool(connection, account.publicKey, account.account)
  );
}
...
export function PoolList({ limit = Infinity }: { limit?: number }) {
  ...
  const sortedPools = React.useMemo(() => {
    return pools.sort((a, b) => {
      ...
      const aTvl =
        (a.account.liquidityCheckpoint * aMeta.usdPrice) / 10 ** aMeta.decimals;
      const bTvl =
        (b.account.liquidityCheckpoint * bMeta.usdPrice) / 10 ** bMeta.decimals;
      ...
      const liqudityDiff =
        b.account.liquidityCheckpoint - a.account.liquidityCheckpoint;
        ...
        {isLoadingPools ? (
          ...
          <>
            {sortedPools.slice(0, limit).map(pool => (
              <PoolTableRow key={pool.publicKey.toBase58()} pool={pool} />
            ))}
          </>
        )}
      </Table.Body>
