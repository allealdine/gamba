...
export default function CustomRenderer() {
  ...
  const VolatilityStars = ({ count }: { count: number }) => {
    ...
    const getShadowClass = (index: number) => {
      return index < count ? "shadow-lg" : "shadow-xs";
    };
    ...
    const emptyStars = Array.from({ length: totalStars - count }).map(
      (_, index) => (
        <FaStar
          key={`empty-${index}`}
          className={`text-gray-300 ${getSizeClass(index)} shadow-xs`}
        />
      ),
    );
  ...
  return (
    <>
      ...
      <div className="w-full relative grid gap-1">
        ...
        <div className="relative grow bg-[#1A1B28] rounded-lg overflow-hidden transition-height duration-200 md:min-h-[550px] min-h-[500px]">
          <GambaUi.PortalTarget target="error" />
          <GambaUi.PortalTarget target="screen" />
          <div className="absolute left-0 bottom-0 p-1 flex gap-2">
            <div className="flex gap-2 justify-center">
              <button
                className="p-2 text-white bg-transparent hover:bg-[#ffffff22] rounded-lg cursor-pointer focus:outline-hidden"
